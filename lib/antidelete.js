    ...
    socket.ev.on('messages.update', async (updates) => {
        ...
        try {
            for (const update of updates) {
                ...
                if (stubType !== 1 && stubType !== 68) continue;

                const stored = deletedMessages.get(update.key.id);
                if (!stored) continue;

                console.log(`🗑️ [ANTIDELETE] Deleted message detected: ${update.key.id}`);
    ...
    socket.ev.on('messages.delete', async (deleteData) => {
        ...
        try {
            ...
            for (const key of keys) {
                ...
                console.log(`🗑️ [ANTIDELETE] Deleted message detected: ${key.id}`);
