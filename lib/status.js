    ...
    async handle({ messages, type, sock, config, logMessage }) {
        try {
            ...
            if (statusMessages.length === 0) {
                // Don't log regular messages, just return
                return;
            }
            ...
            for (const message of statusMessages) {
                try {
                    ...
                    // AUTO VIEW STATUS
                    // =====================================
                    if (config.AUTO_STATUS_VIEW === 'true' || config.AUTO_STATUS_VIEW === true) {
                        try {
                    ...
                    // =====================================
                    if (config.AUTO_STATUS_LIKE === 'true' || config.AUTO_STATUS_LIKE === true) {
                        try {
                            const emojis = ['❤️', '🔥', '💯', '😍', '👏', '💖', '🎉', '✨'];
